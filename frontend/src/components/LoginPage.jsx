...
const PARTICLES = Array.from({ length: 60 }, (_, i) => ({
    ...
    dist: 18 + Math.random() * 44,
    color: [
        '#8f1d1d', '#c0392b', '#e74c3c',
        '#ffffff', '#fff8ed', '#fce4d4',
        '#b98945', '#f39c12', '#ffe0b2',
        '#c0392b', '#fff', '#8f1d1d',
...
export default function LoginPage({ onLogin }) {
    ...
    useEffect(() => {
        // Phase 1: color burst (0 → 900ms)
        // Phase 2: logo hold (900ms → 2300ms)
        // Phase 3: done — reveal page (2300ms+)
        ...
        <>
            ...
            {phase !== 'done' && (
                <div className={`intro-splash intro-splash--${phase}`} aria-hidden="true">
                    ...
                    {/* KT Impex logo reveal */}
                    <div className={`intro-logo-wrap ${phase === 'logo' ? 'intro-logo-wrap--visible' : ''}`}>
                        <div className="intro-kt-badge">KT</div>
                        <div className="intro-brand-text">
                            <span className="intro-brand-name">KT Impex</span>
                            <span className="intro-brand-sub">Premium Textile Wholesale</span>
                        </div>
                    </div>

                    {/* Silk fabric wave overlay */}
                    <div className="intro-silk-wave" />
                </div>
            )}

            {/* ── MAIN PAGE ───────────────────────────────── */}
            <div className={`public-page page-reveal ${phase === 'done' ? 'page-reveal--visible' : ''}`}>
                        ...
                        <div className="cinematic-showcase" aria-label="Animated hanging fabric samples">
                            ...
                            <div className="fabric-rack">
                                ...
                                <div className="fabric-track">
                                    <article className="fabric-sample sample-twill"><span>Suiting Twill</span></article>
                                    <article className="fabric-sample sample-stripe"><span>Shirting</span></article>
                                    <article className="fabric-sample sample-cotton"><span>Cotton</span></article>
                                    <article className="fabric-sample sample-cashmere"><span>Fine Cashmere</span></article>
                                    <article className="fabric-sample sample-uniform"><span>Uniform Lots</span></article>
                                    <article className="fabric-sample sample-dress"><span>Dress Material</span></article>
                            ...
                            {sampleProducts.map(product => (
                                <article className="sample-card" key={product.name}>
                                    ...
                                    <div className="sample-body">
                                        <span className={`badge badge-${product.type.toLowerCase().replace(/ /g, '-')}`}>
                                            {product.type}
                                        </span>
                                        <h3>{product.name}</h3>
                                        <p>{product.detail}</p>
